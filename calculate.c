...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2020/11/12 20:51:51 by mellie            #+#    #+#             */
/*   Updated: 2020/11/12 20:57:48 by mellie           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "fractol.h"

__uint32_t	ft_color(int i, int i_max)
{
	__uint32_t	color;
	int			r;
	int			g;
...
int			burningship(t_fractal *data)
{
	...
	i = 0;
	c.re = ((data->x - data->w / 2) / data->zoom) + data->move_x;
	c.im = ((data->y - data->h / 2) / data->zoom) + data->move_y;
	z = c;
...
int			julia(t_fractal *data)
{
	...
	i = 0;
	z.re = ((data->x - data->w / 2) / data->zoom) + data->move_x;
	z.im = ((data->y - data->h / 2) / data->zoom) + data->move_y;
	while (i < data->iterations && ((z.re * z.re + z.im * z.im) <= 4.f))
	{
		tmp = z;
		z.re = tmp.re * tmp.re - tmp.im * tmp.im;
